...
def _chapters_to_rows(chapters: list[Chapter]) -> list[list[str]]:
    return [
        [_format_time(ch.start), _format_time(ch.end), ch.title or "", ", ".join(ch.keywords or []), ch.summary or "", ch.text]
        for ch in chapters
    ]
...
def build_app(config: AppConfig | None = None) -> gr.Blocks:
    ...
    def run_pipeline(file_path: str, describe_chapters: bool):
        ...
        chapters = pipeline.run(file_path, describe_chapters=describe_chapters)
    ...
    with gr.Blocks(title="Lecture Segmentation Pipeline") as demo:
        ...
        output_table = gr.Dataframe(
            headers=["start", "end", "title", "keywords", "summary", "text"],
            label="Detected chapters",
            wrap=True,
        )
        run_button.click(fn=run_pipeline, inputs=[file_path, describe_chapters], outputs=output_table)
